...
app.post("/", async function(req, res) {
    global.phoneFrom = req.body.phoneFrom;
    global.amount = req.body.amount;
    global.phoneTo = req.body.phoneTo;
    global.code = req.body.code;

    if(global.phoneFrom !== "" && global.amount !== "" && global.phoneTo !== "" ) {

        // Initiate sending money 
        sendMoney(global.phoneFrom, global.amount);
...
exports.callback = functions.https.onRequest(async function(req, res) {
    ...
    if(req.body.Body.stkCallback.ResultCode === 0 && global.phoneTo !== ''){
        var newAmount = 1.2 * parseInt(global.amount);

        console.log("============"+ newAmount);
        
        await sendAirtime(global.phoneTo, global.newAmount, global.code);

        console.log("Status: Sent AIRTIME Successfully!");

    } else {
        console.log('User Cancelled Request OR an ERROR occured');
    }
